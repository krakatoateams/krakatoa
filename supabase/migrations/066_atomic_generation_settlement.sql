-- 066_atomic_generation_settlement.sql
-- Atomic stop settlement + workflow provider commit (Phase 2 control plane).
...
-- eligibility atomically, refunds via ledger RPC when eligible, then closes
-- assets/requests/job. Terminal replay returns existing state — never
-- double-refunds.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- krakatoa_mark_workflow_provider_committed
-- Atomically marks workflow-era provider commit. No-op (returns false) when
-- stop already requested, job terminal, or backend is not workflow.
-- Never clears cancel_requested.
-- ---------------------------------------------------------------------------
create or replace function public.krakatoa_mark_workflow_provider_committed(
  p_profile_id uuid,
  p_job_id uuid,
...
begin
  ...
  if v_req.provider_committed_at is not null then
    return true;
  end if;

  update public.generation_requests
  set
    provider_committed_at = now(),
