-- 074_atomic_generation_request_takeover.sql
-- Reset all attempt-scoped state and provider fences before reusing a failed
-- generation request. Active/recoverable/succeeded linked jobs block takeover.
...
revoke execute on function public.krakatoa_take_over_generation_request(
  uuid, uuid, timestamptz, text, text, text, timestamptz
) from public, anon, authenticated;
grant execute on function public.krakatoa_take_over_generation_request(
  uuid, uuid, timestamptz, text, text, text, timestamptz
) to service_role;
