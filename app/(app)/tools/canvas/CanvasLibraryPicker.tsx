...
const CanvasLibraryContext = createContext<LibraryCtx | null>(null);

export function useCanvasLibrary(): LibraryCtx {
  const ctx = useContext(CanvasLibraryContext);
  if (!ctx) {
    throw new Error("useCanvasLibrary must be used inside CanvasLibraryProvider");
  }
  return ctx;
}

export function CanvasLibraryProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <CanvasLibraryContext.Provider value={{ openLibrary, pickerOpen: !!request }}>
      {children}
        ...
        ? createPortal(
            <div className="fixed inset-0 z-[70] flex justify-end bg-black/55 backdrop-blur-sm">
              ...
              <aside
                role="dialog"
                aria-modal="true"
                aria-labelledby="canvas-library-title"
