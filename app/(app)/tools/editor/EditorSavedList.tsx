...
export default function EditorSavedList({
  open,
  activeId,
  onClose,
  onSelect,
  ...
  const [items, setItems] = useState<EditorSummary[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = (await res.json().catch(() => ({}))) as {
        projects?: EditorSummary[];
        error?: string;
      };
      if (res.status === 401) {
        setError("Sign in to see saved edits.");
        setItems([]);
        return;
      }
      if (!res.ok) throw new Error(data.error || "Failed to load edits.");
  ...
  const beginRename = (item: EditorSummary) => {
  ...
  const commitRename = async () => {
    ...
    try {
      ...
      setItems((list) =>
        list.map((item) =>
          item.id === data.project!.id ? { ...item, title: data.project!.title } : item
        )
      );
      onRenamed?.(data.project.id, data.project.title);
      setRenamingId((active) => (active === id ? null : active));
      setRenameValue("");
    } catch (err) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[70] flex justify-end bg-black/55 backdrop-blur-sm">
      <button type="button" aria-label="Close saved edits" className="absolute inset-0 cursor-default" onClick={onClose} />
      <aside
        role="dialog"
        aria-modal="true"
        aria-labelledby="editor-saved-title"
      ...
      >
        <header className="flex h-14 shrink-0 items-center justify-between gap-3 border-b border-white/10 px-4">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <h2 id="editor-saved-title" className="truncate text-sm font-semibold text-text-primary">
              Saved edits
            </h2>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
          {loading ? (
            ...
            <p className="text-sm text-text-secondary">
              {error ?? "No saved edits yet. Add clips, then Save."}
            </p>
          ) : (
            <>
