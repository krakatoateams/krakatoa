...
/** Build 42 cells (6 rows × 7 cols) for the given month */
...
/** YYYY-MM-DD in local timezone for a Date object */
function toLocalDateKey(date: Date): string {
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
}

/** YYYY-MM-DD in local timezone for a UTC ISO string */
...
export default function CalendarPage() {
  ...
  const handleDrop = useCallback(async (targetDateKey: string) => {
    ...
    // Build new ISO timestamp: keep original time, change the date
