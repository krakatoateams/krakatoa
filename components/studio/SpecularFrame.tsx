...
export type SpecularFrameProps = HTMLAttributes<HTMLDivElement> &
  Partial<ShaderProps> & {
    children: ReactNode;
  };
...
export function SpecularFrame({
  ...
  useEffect(() => {
    ...
    const onVisibility = () => {
      if (!document.hidden && visible) ensureLoop();
    };
  ...
  return (
    <div ref={frameRef} {...rest} className={`relative ${className}`}>
      <div
        ref={fxRef}
        aria-hidden
        className="pointer-events-none absolute z-[1] [&_canvas]:block [&_canvas]:h-full [&_canvas]:w-full"
        style={{ inset: -PAD }}
      />
