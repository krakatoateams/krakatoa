...
/**
 * Core sign-up form — Google OAuth + email/password, with the duplicate-
 * account error states. Shared between the full-page `/signup` route and
 * SignInModal's "signup" view, mirroring SignInForm's structure exactly.
 */
export function SignUpForm({
  next = "/dashboard",
  /**
   * When set (i.e. rendered inside SignInModal), "Log in here" swaps the
   * modal's view in-place instead of navigating. Omit this to fall back to
   * a real link to /login — used by the standalone /signup page, which has
        ...
        {signupError?.kind === "duplicate_email" &&
          (onSwitchToSignIn ? (
            <div className="rounded-radius-md border border-error/30 bg-error/10 px-3 py-2.5 text-small text-error">
              ...
              <button type="button" onClick={onSwitchToSignIn} className="underline hover:text-white">
                Log in here
              </button>
            </div>
          ) : (
